...
export const UserManagement: React.FC = () => {
  ...
  const deleteUser = async (id: string) => {
    if (
      !window.confirm(
        "Are you sure you want to permanently drop this profile context?",
      )
    )
      return;
    try {
      ...
      toast.error(
        err.response?.data?.message ||
          "Failed to drop staff directory mapping.",
      );
    }
  };
    ...
    <div className="space-y-6 w-full font-sans text-slate-800 pb-16">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs font-medium text-slate-400 mt-0.5">
              Configure operational staff profiles and security matrix layers.
            </p>
          </div>
        </div>
      ...
      <div className="bg-white rounded-3xl shadow-3xs border border-slate-100 overflow-hidden">
        <div className="p-5 border-b border-slate-100 bg-white flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h3 className="font-black text-xl tracking-tight flex items-center gap-2 text-slate-990">
              <Table size={20} className="text-slate-400" /> System Access
              Directories
            </h3>
            <p className="text-xs text-slate-400 mt-0.5">
              Modify operational roles, parameters, and status vectors.
            </p>
          </div>
          ...
          <table className="w-full text-left border-collapse min-w-[900px]">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-100 text-[10px] font-black uppercase tracking-widest text-slate-400 select-none">
                <th
                  className="py-4 px-6 cursor-pointer hover:bg-slate-100/60 transition-colors w-1/4"
                ...
                >
                  <div className="flex items-center gap-1.5">
                    <Mail size={12} className="shrink-0" /> Email Address{" "}
                    <ArrowUpDown
                      size={12}
                      className="text-slate-400 shrink-0"
                    />
                ...
                <th
                  className="py-4 px-6 cursor-pointer hover:bg-slate-100/60 transition-colors w-1/5"
                  onClick={() => toggleSort("mobile")}
                >
                  <div className="flex items-center gap-1.5">
                    <Phone size={12} className="shrink-0" /> Contact Number{" "}
                    <ArrowUpDown
                      size={12}
                      className="text-slate-400 shrink-0"
                    />
                ...
                <th
                  className="py-4 px-6 cursor-pointer hover:bg-slate-100/60 transition-colors"
                  onClick={() => toggleSort("roleId.name")}
                >
                  <div className="flex items-center gap-1.5">
                    <Shield size={12} className="shrink-0" /> Authorization Role
                    / Store Assignment{" "}
                    <ArrowUpDown
                      size={12}
                      className="text-slate-400 shrink-0"
              ...
              {paginatedData.length > 0 ? (
                paginatedData.map((u) => (
                  ...
                  >
                    <td className="py-4 px-6">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="truncate">
                          <span className="font-bold text-slate-800 tracking-tight block truncate">
                    ...
                    <td className="py-4 px-6 font-medium text-slate-600 font-mono text-xs truncate">
                      {u.email}
                    </td>
                    ...
                    <td className="py-4 px-6">
                      <div className="flex flex-col items-start gap-1">
                        <span className="inline-flex items-center px-2.5 py-0.5 rounded-lg text-[10px] font-black uppercase tracking-wide bg-red-50 text-[#7f1d1d] border border-red-100/40">
                          {u.roleId?.name || "Unassigned Staff"}
                        </span>

                        {/* ✅ ADVANCED LOCATION RENDERING ENGINE CELL: Renders store name, code, city and area dynamically if present */}
                        {u.storeId && (
                          <div className="mt-1 flex flex-col gap-0.5">
                            ...
                            {/* <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest block font-mono pl-4">
                              📍 {u.storeId.city || "No City"}{" "}
                              {u.storeId.area ? `• ${u.storeId.area}` : ""}
                            </span> */}
                          </div>
                        )}
                      </div>
                    ...
                    <td className="py-4 px-6 text-center">
                      <span
                        className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-lg text-[9px] font-black uppercase tracking-wider border ${
                          u.status === "active"
                ...
                <tr>
                  ...
                  >
                    No matching registry references allocated in this workspace
                    index.
                  </td>
                </tr>
              )}
