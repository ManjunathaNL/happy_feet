...
const initialState: AuthState = {
  ...
  authModalType: 'login',
  currentTheme: JSON.parse(localStorage.getItem('hf_theme') || '{"bg": "#7f1d1d", "text": "#ffffff"}'),
};
...
const authSlice = createSlice({
  ...
  reducers: {
    ...
    setAuthSuccess: (state, action: PayloadAction<{ token: string; user: AuthUser; routes: RouteAccess[] }>) => {
    ...
    updateThemeStyles: (state, action: PayloadAction<{ bg: string; text: string }>) => {
      state.currentTheme = action.payload;
      localStorage.setItem('hf_theme', JSON.stringify(action.payload));
      document.documentElement.style.setProperty('--dynamic-accent-bg', action.payload.bg);
      document.documentElement.style.setProperty('--dynamic-accent-text', action.payload.text);
    },
    logout: (state) => {
      state.token = null;
      state.user = null;
      ...
      state.isAuthModalOpen = false;
      localStorage.clear();
      document.documentElement.style.setProperty('--dynamic-accent-bg', '#7f1d1d');
      document.documentElement.style.setProperty('--dynamic-accent-text', '#ffffff');
    }
  }
});

export const { 
  setAuthStart, setAuthSuccess, setAuthFailure, 
  openAuthModal, closeAuthModal, updateThemeStyles, logout 
} = authSlice.actions;
